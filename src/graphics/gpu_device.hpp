...
namespace puffin {
    ...
    struct GPUTimestampManager {
        ...
        bool            has_valid_queries() const; // means it won't change the state of the object called via
        void            reset();
        u32             resolve(u32 current_frame, GPUTimestamp* timestamps_to_fill); // Returns total queries this frame

        u32             push(u32 current_frame, cstring name);
        ...
        // Used to query the GPU only once per frame if get_gpu_timestamps is called more than once per frame
        bool                    current_frame_resolved = false;
    };

    struct DeviceCreation {
        Allocator*              allocator           = nullptr;
        StackAllocator*         temporary_allocator = nullptr;
        void*                   window              = nullptr; // API=specific window (SDL or GLFW window)
        u16                     width               = 1;
        u16                     height              = 1;

        u16                     gpu_time_queries_per_frame = 32;
        bool                    enabled_gpu_time_queries = false;
        bool                    debug               = false;
    ...
    struct GpuDevice : public Service {
        ...
        void                    destroy_render_pass(RenderPassHandle pass);
        void                    destroy_shader_state(ShaderStateHandle state);
        ...
        void                    query_descriptor_set(DescriptorSetHandle layout, DescriptorSetDescription& out_description);
        void                    query_shader_state(ShaderStateHandle layout, ShaderStateDescription& out_description);
        ...
        void                    set(PresentMode::Enum mode);

        void                    fill_barrier(RenderPassHandle render_pass, ExecutionBarrier& out_barrier);

        BufferHandle            get_fullscreen_vertex_buffer() const; // returns a vertex buffer usable for fullscreen shaders that use no vertices
        ...
        static constexpr cstring k_name                         = "puffin_gpu_service";
        ...
        // These are dynamic - so that workload can be handled easily
