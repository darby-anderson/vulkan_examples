...
void CommandBuffer::bind_index_buffer(BufferHandle buffer_handle, u32 offset_) {
    Buffer* buffer = device->access_buffer(buffer_handle);
...
void CommandBuffer::bind_descriptor_set(DescriptorSetHandle* descriptor_set_handles, u32 num_lists, u32* offsets,
                                        u32 num_offsets) {
    u32 offsets_cache[8];
    num_offsets = 0;
...
void CommandBuffer::set_viewport(const Viewport* viewport) {
    ...
    if(viewport) {
        ...
        // Invert Y with negative height and proper offset - vulkan clips y uniquely
...
void CommandBuffer::set_scissors(const Rect2DInt* rect) {
    VkRect2D vk_scissor;
...
void CommandBuffer::draw_indexed(TopologyType::Enum topology, u32 index_count, u32 instance_count, u32 first_index,
...
void CommandBuffer::dispatch(u32 group_x, u32 group_y, u32 group_z) {
    // Used for starting compute shader work
...
void CommandBuffer::draw_indexed_indirect(BufferHandle buffer_handle, u32 offset, u32 stride) {
    ...
    vkCmdDrawIndexedIndirect(vk_command_buffer, vk_buffer, vk_offset, 1, sizeof(VkDrawIndexedIndirectCommand));
}
...
void CommandBuffer::dispatch_indirect(BufferHandle buffer_handle, u32 offset) {
    ...
    vkCmdDispatchIndirect(vk_command_buffer, vk_buffer, offset);
}

static ResourceState to_resource_state(PipelineStage::Enum stage) {
    static ResourceState s_states[] =
            {RESOURCE_STATE_INDIRECT_ARGUMENT, RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER,
             RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, RESOURCE_STATE_PIXEL_SHADER_RESOURCE,
             RESOURCE_STATE_RENDER_TARGET, RESOURCE_STATE_UNORDERED_ACCESS, RESOURCE_STATE_COPY_DEST};
...
void CommandBuffer::barrier(const ExecutionBarrier& barrier) {
    ...
    // if we are using the experimental buffer
    if(barrier.new_barrier_experimental != u32_max) {
        ...
        for(u32 i = 0; i < barrier.num_image_barriers; i++) {
...

            {
                ...
                ResourceState current_state = barrier.source_pipeline_stage == PipelineStage::RenderTarget ?
                        RESOURCE_STATE_RENDER_TARGET : RESOURCE_STATE_PIXEL_SHADER_RESOURCE;

                ResourceState next_state = barrier.destination_pipeline_stage == PipelineStage::RenderTarget ?
                        RESOURCE_STATE_RENDER_TARGET : RESOURCE_STATE_PIXEL_SHADER_RESOURCE;

                if(!is_color) {
                    current_state = barrier.source_pipeline_stage == PipelineStage::RenderTarget ?
                            RESOURCE_STATE_DEPTH_WRITE : RESOURCE_STATE_DEPTH_READ;
                    next_state = barrier.destination_pipeline_stage == PipelineStage::RenderTarget ?
                            RESOURCE_STATE_DEPTH_WRITE : RESOURCE_STATE_DEPTH_READ;
                }
        ...
        source_stage_mask = util_determine_pipeline_stage(source_access_flags, barrier.source_pipeline_stage == PipelineStage::ComputeShader ? QueueType::Compute : QueueType::Graphics);
        destination_stage_mask = util_determine_pipeline_stage(destination_access_flags, barrier.destination_pipeline_stage == PipelineStage::ComputeShader ? QueueType::Compute : QueueType::Graphics);

        vkCmdPipelineBarrier(vk_command_buffer, source_stage_mask, destination_stage_mask, 0, 0,
                             nullptr, barrier.num_memory_barriers, buffer_memory_barriers, barrier.num_image_barriers, image_barriers);
        return;
    }
    ...
    for(u32 i = 0; i < barrier.num_image_barriers; i++) {
        ...
        // Transition to:
        vk_barrier.oldLayout = is_color ? new_layout : new_depth_layout;
