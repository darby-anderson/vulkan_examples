...
struct Resource {
    ...
    void        remove_reference() { PASSERT(references != 0); references--; }
...
struct ResourceLoader {
    ...
    virtual     Resource*           create_from_file(cstring name, cstring filename, puffin::ResourceManager* resource_manager) { return nullptr; }
};
...
inline T* ResourceManager::load(cstring name) {
    ...
    if(loader) {
        // search if resource is already in cache
